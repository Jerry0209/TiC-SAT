// Auto generated C++ code started by /workspace/gem5-X-TiC-SAT/src/mem/slicc/symbols/StateMachine.py:1188
// DMA: DMA Controller
                                        ...
                                        Addr addr)
{
  ...
  case HASH_FUN(DMA_State_READY, DMA_Event_ReadRequest):
    ...
    if (!(*m_reqToDir_ptr).areNSlotsAvailable(1, clockEdge()))
  ...
  case HASH_FUN(DMA_State_BUSY_RD, DMA_Event_Inv_Ack):
  case HASH_FUN(DMA_State_BUSY_WR, DMA_Event_Inv_Ack):
  case HASH_FUN(DMA_State_BUSY_WR, DMA_Event_DMA_Ack):
    if (!(*m_triggerQueue_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    u_updateAckCount(m_tbe_ptr, addr);
    o_checkForCompletion(m_tbe_ptr, addr);
  ...
  case HASH_FUN(DMA_State_BUSY_RD, DMA_Event_Data):
    ...
    d_dataCallbackFromTBE(m_tbe_ptr, addr);
    w_deallocateTBE(m_tbe_ptr, addr);
    p_popResponseQueue(m_tbe_ptr, addr);
    wkad_wakeUpAllDependents(m_tbe_ptr, addr);
    return TransitionResult_Valid;

  case HASH_FUN(DMA_State_BUSY_RD, DMA_Event_All_Acks):
    next_state = DMA_State_READY;
    d_dataCallbackFromTBE(m_tbe_ptr, addr);
    w_deallocateTBE(m_tbe_ptr, addr);
    p_popTriggerQueue(m_tbe_ptr, addr);
    wkad_wakeUpAllDependents(m_tbe_ptr, addr);
    return TransitionResult_Valid;
  ...
  case HASH_FUN(DMA_State_BUSY_WR, DMA_Event_All_Acks):
    next_state = DMA_State_READY;
    if (!(*m_respToDir_ptr).areNSlotsAvailable(1, clockEdge()))
        return TransitionResult_ResourceStall;
    a_ackCallback(m_tbe_ptr, addr);
    u_sendExclusiveUnblockToDir(m_tbe_ptr, addr);
    w_deallocateTBE(m_tbe_ptr, addr);
    p_popTriggerQueue(m_tbe_ptr, addr);
    wkad_wakeUpAllDependents(m_tbe_ptr, addr);
    return TransitionResult_Valid;
